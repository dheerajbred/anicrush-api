...
| `GET`  | `/anime/genres/:genreTag`             | Get anime by genre with limit.                   | `genreTag` (required), `page` (1), `limit` (24)  |
...
### Get anime by genre with limit
```bash
curl "http://localhost:3001/api/anime/genres/action?limit=24&page=1"
```
